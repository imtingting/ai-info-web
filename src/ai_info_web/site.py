...
def _index_page(products, statuses, timestamp: datetime) -> str:
    ...
    trending_observation = (
        f"汇总本周 GitHub Trending 榜单中值得关注的项目，抓取于 {_display_date(trending[0]['trending_observed_at'])} UTC。"
        if trending
        else "本周 Trending 榜单还在整理中，先去 GitHub 查看实时榜单。"
    )
    trending_source = (
        '<a href="https://github.com/trending?since=weekly" target="_blank" rel="noopener noreferrer">查看来源</a>'
        if trending
        else ""
    )
    trending_markup = trending_cards or (
        '<div class="trending-empty"><p class="empty">本周 Trending 榜单还在整理中，先去 GitHub 查看实时榜单。</p>'
        '<a class="source-link" href="https://github.com/trending?since=weekly" target="_blank" rel="noopener noreferrer">查看 GitHub Trending</a></div>'
    )
    return _page_shell(
...
<section class="detail-section chat" data-chat data-endpoint="{html.escape(chat_endpoint or '', quote=True)}" data-product-slug="{html.escape(str(product['slug']), quote=True)}">
  ...
  <form data-chat-form><textarea name="message" maxlength="1000" rows="3" placeholder="输入关于该项目的问题"{chat_disabled}></textarea><div class="chat-actions"><label class="chat-web-toggle"><input type="checkbox" name="use_web"{chat_disabled}> 联网检索</label><span data-chat-error role="status"></span><button type="submit"{chat_disabled}>发送</button></div></form>
    ...
    window = f"近 {window_days} 天" if window_days is not None else "暂无增长数据"
    return _page_shell(
...
def _heat_evidence(breakdown: dict) -> list[str]:
    ...
    if isinstance(github, dict):
        ...
        evidence.append(
            f"近 {window} 天 GitHub 增长明显：Stars +{raw.get('stars_delta') or 0}，Forks +{raw.get('forks_delta') or 0}。"
        )
...
body{background:radial-gradient(circle at 72% 18%,rgba(107,224,194,.045),transparent 28%),#0d1316}.page-particles{position:fixed;inset:0;width:100vw;height:100vh;z-index:0;opacity:.82;pointer-events:none}.topbar,main{position:relative;z-index:1}.overview{min-height:310px;padding-top:74px;padding-bottom:58px;background:transparent;border-bottom-color:rgba(66,92,101,.54)}.overview::before,.overview::after{content:none;animation:none}.overview h1{text-shadow:0 0 22px rgba(107,224,194,.1)}.overview .eyebrow{text-shadow:0 0 12px rgba(107,224,194,.34)}.hero-particles{display:none}.product-card,.metrics div,.chat form,.chat-messages,.chat-empty{backdrop-filter:blur(3px)}.trending-empty{display:flex;align-items:center;gap:12px;flex-wrap:wrap;padding:18px 0}.trending-empty .empty{margin:0}.trending-empty .source-link{margin:0}@media(prefers-reduced-motion:reduce){.page-particles{display:none}}@media(max-width:620px){.overview{min-height:280px;padding-top:48px;padding-bottom:42px}.trending-empty{align-items:stretch;flex-direction:column}.trending-empty .source-link{width:max-content}}
"""
